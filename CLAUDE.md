...
`docker-rbac/` is the step beyond that mock: a Docker Compose file that actually boots a minimal, real Confluent Platform broker with RBAC and MDS enabled (single-node KRaft, FILE-based MDS user store instead of LDAP, a locally-generated token-signing keypair, no TLS), and `docker-rbac/test_rbac.sh` runs *both* sources — `examples/cc-acls.yaml` directly, and `examples/ranger-kafka-policies.json` via `export_ranger_policies.py` first — through the real `acl_to_rolebindings.py --target cp-mds` → `apply_to_mds.py --apply` pipeline against it, then re-reads every binding back from MDS (`docker-rbac/verify_bindings.py`, using MDS's own `roleNames`/`resources` lookup endpoints) rather than trusting `apply_to_mds.py`'s 204s alone. This is what actually confirmed `targets/cp_mds.py`'s two-endpoint split works for both sources, not just that it matches the docs. Non-obvious gotchas documented in `docker-rbac/README.md`: the FILE user store parser rejects `#` comment lines (so `login.properties` has none); MDS 400s a cluster-scoped bind for a role like `ResourceOwner` that actually needs resources (`"Resources must be specified for role ResourceOwner"`) — that's the server validating correctly, not a bug in this repo; MDS's *read* endpoints (`lookup/principals/.../roleNames`, `principals/.../roles/.../resources`) take a bare `{"clusters": {...}}` body with no `"scope"` wrapper, unlike the write endpoints; and several cp-server internal topics/features (license, tier-metadata, balancer, cluster-linking, offsets, RBAC metadata) default to replication factor 3 and need forcing to 1 for a single-node cluster — `confluent-audit-log-events` is the one exception left alone, since the broker already self-heals its replication factor down to 1 on its own.
