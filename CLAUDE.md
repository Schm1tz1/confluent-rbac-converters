...
```bash
python3 -m pip install PyYAML   # only dependency
...
3. **Apply (target-specific, one script per target).** `apply_rolebindings.py` POSTs each `--target cc` binding's `{principal, role_name, crn_pattern}` to `https://api.confluent.cloud/iam/v2/role-bindings`; `apply_to_mds.py` POSTs each `--target cp-mds` binding's `{principal, role_name, scope, resource_patterns}` to an MDS role-binding endpoint (`{mds_url}/security/1.0/principals/{principal}/roles/{role}/bindings` — **best-guess wire format, unverified against a live MDS instance; confirm before `--apply`**). Both share `common/apply_shell.py` for the generic dry-run/retry/backoff/`--continue-on-error`/results-log shell (dry-run is the default; `--apply` is required to actually POST; retries with backoff on 429/5xx honoring `Retry-After`); only the per-target payload shape, auth, and endpoint construction live in `targets/*.py`.
